...
class SpellList(Screen):
    ...
    def __init__(self):
        ...
        self._search_bar = TextBox((380, 400), (200, 50), "", label="Search Spell(s)")

    def _draw(self, screen):
        spell = ''
        for spellsname, info in self.spell_list.items():
            if self._search_bar.value.lower() in spellsname.lower() or self._search_bar.value == str(info['level']):
